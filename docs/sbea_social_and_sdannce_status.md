...
- 🟡 **BA 부재 = 잔차 격차의 유력 후보** (미검정). 저자는 `bundle_adjustment.pyd` 를 쓰고
  우리 DLT 는 안 쓴다. S2 보다 싸게 검정 가능.
...
### S2'. BA 부재 가설 검정 — 저자 코드 불요 (신설, S2 보다 선행)

저자 코드를 돌리지 않고 격차 후보를 직접 친다. **우리 파이프라인에 bundle adjustment 를 붙여**
잔차가 내려가는지 본다.

- 독립 지표 필수 = 본 길이 rCV 유지 여부(§6 함정). 재투영만 내려가면 실패로 판정.
- 자유 파라미터 도입 시 **선택에 안 쓴 세션**에서 검증.

### S2. 저자 pretrained 로 individual 세션 1개 재현 — 비용 재평가됨 (구 1일 → 그 이상)
