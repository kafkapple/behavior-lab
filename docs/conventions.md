...
- **SBeA16 좌우 대칭쌍 5개**(`sbea_lr_resolve.py` — `SYM_PAIRS`): 귀·앞다리·뒷다리·앞발톱·뒷발톱.
  저자 DLC 는 카메라마다 이 쌍의 좌/우를 다르게 붙인다. 20세션 전량 기준 **대칭 관절 잔차
  중앙값 27.8 px 대 중앙선 관절 19.9 px** — 최악 4개가 전부 대칭 관절이다(뒷발톱 57.8/41.4 px).
  즉 좌/우 불일치는 **바닥(≈20 px) 위 초과분의 주원인**이지 잔차 전체의 지배 성분은 아니다.
  바닥 자체는 캘리브레이션 정밀도 + 2D 국소화 오차이고 이 데이터로는 분해되지 않는다.
  완화(`--resolve-lr`)는 재투영은 20/20 낮추지만 본 길이 강성은 14/20·중앙값 −2% 로 거의
  못 따라온다 → **기본 off**. 또한 카메라 간 *일관성*만 보장하며 **어느 쪽이 해부학적
  왼쪽인지는 검증되지 않았다**(검출기 다수결 승계). 근거 = `scripts/sweep_sbea_sessions.py`.
