...
def load(ds):
    if ds=="shot7m2":
        ...
        eps=P[list(P.keys())[0]]; L=np.load(f"{SHOT7M2}/test/benchmark_labels.npy",allow_pickle=True).item()
    ...
    gt=np.load(f"outputs/{ds}/labels.npy")[:8000]; ck=sorted(glob.glob(f"{BMAE_OUT}/{ds}/checkpoint-*.pth"))[-1]
