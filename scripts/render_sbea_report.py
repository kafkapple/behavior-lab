"""Self-contained HTML report for the SBeA keypoint / mask pipeline.

Inputs are artefacts the pipeline already writes, so this adds no state:
  --npz          scripts/sbea_dlc_triangulate.py output
  --masks        kp-SAM2 tree (Camera1..4)
  --masks-sam3   auto_sam3 tree (cam0..3)
  --ba-npz-dir   kp3d dir; fits the extrinsic BA correction for the before/after panel

Figures live in `sbea_report_figs.py`.

Usage:
    python scripts/render_sbea_report.py --npz <session>-kp3d.npz \
        --video-root /mnt/d/data/raw/SBeA/individual --out outputs/sbea_report.html
"""
...
from sbea_report_figs import (  # noqa: E402
    REGION_LEGEND, cam_dir, fig_mask_area, fig_residual, fig_skeleton, mismatch_rows,
    overlay_2d, overlay_mask, overlay_reproj, project, read_mask, _tri,
)
...
def main() -> None:
    ...
    mrows, n_flag = mismatch_rows(resid, names)
    legend = " ".join(
        f'<span style="display:inline-block;width:.7em;height:.7em;border-radius:50%;'
        f'background:rgb({r},{g},{b});margin:0 .25em 0 .8em"></span>{lab}'
        for lab, (r, g, b) in REGION_LEGEND)

    valid = np.isfinite(kp3d).all(axis=2)
    a.out.parent.mkdir(parents=True, exist_ok=True)
    a.out.write_text(TEMPLATE.format(
        mrows=mrows, n_flag=n_flag, legend=legend,
...
<h2>2. 2D 검출 — 카메라별</h2>
<p class="sub">저자 DLC 스냅샷의 출력. 3D 삼각측량의 <b>입력</b>이다 — 점이 쥐 위에 있으면 정상.<br>
색상 규약 = BehaviorSplatter <code>KP_COLORS</code> 부위별 팔레트(좌/우 다른 색이라 <b>좌우 뒤바뀜이 색으로 보인다</b>):
{legend}<br>흰 테두리 = likelihood 임계 미만.</p>
...
<h2>4.5 정량 불일치 탐지 — 관절별</h2>
<p class="sub">중앙선 관절(좌/우 모호성이 원리적으로 없는 관절)의 중앙값을 <b>바닥</b>으로 두고 각 관절을 비교한다.
<b>OUTLIER</b> = 바닥의 1.5배 초과 · <b>L/R GAP</b> = 좌우 짝의 값이 30% 이상 벌어짐(카메라별 라벨 스왑의 서명).
현재 <b>{n_flag}/{K}</b> 관절이 플래그됨.</p>
<table><tr><th>관절</th><th>median (px)</th><th>바닥 대비</th><th>좌우 짝</th><th>플래그</th></tr>{mrows}</table>
